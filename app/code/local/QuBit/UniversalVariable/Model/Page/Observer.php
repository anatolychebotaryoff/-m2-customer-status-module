...
class QuBit_UniversalVariable_Model_Page_Observer {
  ...
  protected $_qubit_session = null;

  protected function _getRequest() {
    return Mage::app()->getFrontController()->getRequest();
  }

  protected function _getQubitSession() {
  	if(is_null($this->_qubit_session)) {
  		$this->_qubit_session = Mage::getSingleton('universal_variable_main/session');
    }
    return $this->_qubit_session;
  }
 ...
 */
  ...
  protected function _setOfferId(){
	  $_session = $this->_getQubitSession();
	  if($this->_getRequest()->getParam('offer_id')) {
		  $_session->setOfferId($this->_getRequest()->getParam('offer_id'));
	  }
  }

  public function getOfferId() {
  	$_session = $this->_getQubitSession();
    $offer_id = $_session->getOfferId();
  	return $offer_id;
  }

	protected function _setAffiliateId() {
		$_session = $this->_getQubitSession();
		if($this->_getRequest()->getParam('affiliate_id')) {
			$_session->setAffiliateId($this->_getRequest()->getParam('affiliate_id'));
		}
	}

  public function getAffiliateId() {
	  $_session = $this->_getQubitSession();
	  $affiliate_id = $_session->getAffiliateId();
	  return $affiliate_id;
  }

	protected function _setTransactionId() {
		$_session = $this->_getQubitSession();
		if($this->_getRequest()->getParam('transaction_id')) {
			$_session->setTransactionId($this->_getRequest()->getParam('transaction_id'));
		}
	}

  public function getTransactionId() {
	  $_session = $this->_getQubitSession();
	  $transaction_id = $_session->getTransactionId();
