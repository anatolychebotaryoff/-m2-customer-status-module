    ...
    protected function _applyCustomOption()
    {
        ...
        $select = $write->select()
            ...
            ->join(
                ...
                'gps.entity_id = i.entity_id AND gps.website_id = i.website_id'
                  . ' AND gps.customer_group_id = i.customer_group_id',
                array());
        ...
        $minPriceRound  = new Zend_Db_Expr("ROUND(i.price * ({$optPriceValue} / 100), 4)");
        $minPriceRound  = $write->getCheckSql("gps.isfixed > 0", "ROUND(i.base_group_price)", $minPriceRound);
        ...
        $minPrice       = $write->getCheckSql("gps.isfixed > 0", "ROUND(i.base_group_price)", $minPrice);
        ...
        $tierPrice      = $write->getCheckSql("gps.isfixed > 0", "ROUND(i.base_group_price)", $tierPrice);

        $groupPriceRound = new Zend_Db_Expr("ROUND(i.base_group_price * ({$optPriceValue} / 100), 4)");
        $groupPriceRound = $write->getCheckSql("gps.isfixed > 0", "ROUND(i.base_group_price)", $groupPriceRound);
        $groupPriceExpr  = $write->getCheckSql("{$optPriceType} = 'fixed'", $optPriceValue, $groupPriceRound);
