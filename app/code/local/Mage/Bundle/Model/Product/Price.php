...
class Mage_Bundle_Model_Product_Price extends Mage_Catalog_Model_Product_Type_Price
{
     ...
     * Modified for Hotdeal group pricing
     *
     * @param Mage_Catalog_Model_Product $product
    ...
    protected function _applyGroupPrice($product, $finalPrice)
    {
        ...
        if (is_numeric($groupPrice) && (float)$groupPrice > 0) {

            $gpData = $product->getData('group_price');

            $cgId = $this->_getCustomerGroupId($product);
            ...
            if (is_null($gpData)) {
                ...
                if ($attribute) {
                    ...
                    if (array_key_exists( $cgId, $gpData)) {
                        $gpData = $gpData[$cgId];
                    }
