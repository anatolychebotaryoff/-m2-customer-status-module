...
class Mage_ImportExport_Model_Import_Entity_Product extends Mage_ImportExport_Model_Import_Entity_Abstract
{
    ...
    protected $_particularAttributes = array(
        ...
        '_links_replacedby_sku', '_links_replacedby_position',
    ...
    protected function _saveCustomOptions()
    {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            if ($this->getBehavior() != Mage_ImportExport_Model_Import::BEHAVIOR_APPEND &&
                $this->getBehavior() != Mage_ImportExport_Model_Import::BEHAVIOR_UPDATE) { // remove old data?
    ...
    protected function _saveLinks()
    {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            if (Mage_ImportExport_Model_Import::BEHAVIOR_APPEND != $this->getBehavior() && $productIds) {
                $linkTypeCondition = '';
                if ($this->getBehavior() == Mage_ImportExport_Model_Import::BEHAVIOR_UPDATE) {
                    $linkTypeCondition = $adapter->quoteInto(' AND link_type_id in (?)', array_unique($setLinkIds));
                }
                $adapter->delete(
                    $mainTable,
                    $adapter->quoteInto('product_id IN (?)', array_unique($productIds)) . $linkTypeCondition
                );
            }
            if ($linkRows) {
    ...
    protected function _saveProductCategories(array $categoriesData)
    {
        ...
        if ($categoriesData) {
            ...
            if (Mage_ImportExport_Model_Import::BEHAVIOR_APPEND != $this->getBehavior() &&
                Mage_ImportExport_Model_Import::BEHAVIOR_UPDATE != $this->getBehavior() ) {
                $this->_connection->delete(
                    $tableName,
