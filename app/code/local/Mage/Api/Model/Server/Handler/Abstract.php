...
abstract class Mage_Api_Model_Server_Handler_Abstract
{
    ...
    //lyonscg enazarov start[340901] extends handlePhpError to add the error line to logs.
    public function handlePhpError($errorCode, $errorMessage, $errorFile, $errline)
    {
        Mage::log($errorMessage . $errorFile . ' on line ' . $errline);
    ...
    public function login($username, $apiKey = null)
    {
        ...
        $sid = $this->_getSession()->getSessionId();
        Mage::log("Login User: " . $username . " session " . $sid, null, 'api.log');
        return $sid;
    }
    ...
    public function call($sessionId, $apiPath, $args = array())
    {
        ...
        try {
            ...
            if (is_callable(array(&$model, $method))) {
                Mage::log($sessionId . ' called ' . $method, null, 'api.log'); //logs the method
                if (is_array($args)) {
                    Mage::log($sessionId . ' args ' . json_encode($args), null, 'api.log'); //logs the arguments
                }
