...
class MageWorx_SeoSuite_Controller_Router extends Mage_Core_Controller_Varien_Router_Standard
{
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
        ...
        if (Mage::helper('seosuite')->isLNFriendlyUrlsEnabled() && $this->_matchCategoryPager($request)) {
            return true;
        }

        if (Mage::helper('seosuite')->isLNFriendlyUrlsEnabled() && $this->_matchCategoryLayer()) {
            return true;
        }
            ...
            case 'rss':
                ...
                switch ($t) {
                    ...
                    case 'discounts':
                        ...
                        break;
                    default:
                        $category = Mage::getModel('seosuite/catalog_category')->setStoreId($storeId)->loadByAttribute('url_key',
                            $d[2]);
                        ...
                        if ($product && $product->getId() > 0) {
                            ...
                            if ($canonicalUrl) {
                                if (strpos($canonicalUrl, 'http') === false) {
                                    $canonicalUrl = Mage::getBaseUrl() . ltrim($canonicalUrl, '/');
                                }
                                Mage::app()->getFrontController()->getResponse()->setRedirect($canonicalUrl)->setHttpResponseCode($seoCode)->sendResponse();
                                exit;
                            }
                        }
                    }
    ...
    protected function _matchCategoryPager($request)
    {
        ...
        $urlSplit = explode('/' . Mage::helper('seosuite')->getLayeredNavigationIdentifier() . '/', $identifier, 2);
        if (isset($urlSplit[1])) {
            return false;
        }
