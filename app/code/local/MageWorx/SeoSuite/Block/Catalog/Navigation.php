...
class MageWorx_SeoSuite_Block_Catalog_Navigation extends MageWorx_SeoSuite_Block_Catalog_Navigation_Abstract
{
    ...
    {
        if ($level == 0) {
            $zeroLevelPosition = isset($this->_itemLevelPositions[$level]) ? $this->_itemLevelPositions[$level]
                    + 1 : 1;
            ...
            $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
        ...
        if (!$this->_isLinkTitleEnabled) {
            return parent::_renderCategoryMenuItemHtml($category, $level, $isLast, $isFirst, $isOutermost,
                            $outermostItemClass, $childrenWrapClass, $noEventAttributes);
        }
        $html = '';
        ...
        if ($hasChildren) {
            ...
            foreach ($children as $child) {
                if ($child->getIsActive()) {
                    $htmlChildren.= $this->_renderCategoryMenuItemHtml($child, $level + 1, ++$j >= $cnt, $isFirst,
                            $isOutermost, $outermostItemClass, $childrenWrapClass, $noEventAttributes);
                }
            }
