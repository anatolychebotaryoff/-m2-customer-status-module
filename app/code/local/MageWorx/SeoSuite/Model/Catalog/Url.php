<?php
class MageWorx_SeoSuite_Model_Catalog_Url extends Mage_Catalog_Model_Url
{
    /**
    ...
    protected function _getProductRequestPathSince17ver($product, $category){
        if (!Mage::helper('seosuite')->isOptimizedUrlsEnabled())
        {
        ...
        else
        {
            ...
            if ($category->getLevel() > 1) {
                // To ensure, that category has path either from attribute or generated now
                $this->_addCategoryUrlPath($category);
                $categoryUrl = Mage::helper('seosuite/category')->getCategoryUrlPath($category->getUrlPath(),
                    false, $storeId, true);
                $requestPath = $categoryUrl . '/' . $urlKey;
            } else {
                $requestPath = $urlKey;
