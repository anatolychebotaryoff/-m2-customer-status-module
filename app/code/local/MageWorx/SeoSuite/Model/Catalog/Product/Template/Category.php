 ...
 */
class MageWorx_SeoSuite_Model_Catalog_Product_Template_Category extends MageWorx_SeoSuite_Model_Catalog_Product_Template_Abstract
{
...
//                        break;
                    ...
                    case 'price':
                        ...
                        if ($this->_product->getTypeId() == 'bundle') {
                            ...
                            if ($this->_product->getPriceType() == 1) {
                                ...
                                if ($_weeeTaxAmount && Mage::helper('weee')->typeOfDisplay($this->_product,
                                                array(0, 1, 4))) {
                            ...
                            if ($_minimalPriceTax <> $_maximalPriceTax)
                                    $value = Mage::helper('seosuite')->__('form %s to %s',
                                        Mage::app()->getStore()->convertPrice($_minimalPriceTax, true, false),
                                        Mage::app()->getStore()->convertPrice($_maximalPriceTax, true, false));
                            else $value = Mage::app()->getStore()->convertPrice($_maximalPriceTax, true, false);
                        } elseif ($this->_product->getTypeId() == 'grouped') {
                            $price = $taxHelper->getPrice($this->_product, $this->_product->getMinimalPrice(),
                                    $includingTax);
                            if ($price > 0)
                                    $value = Mage::helper('seosuite')->__('form %s',
                                        Mage::app()->getStore()->convertPrice($price, true, false));
                            else $value = '';
                        } elseif ($this->_product->getPrice() > 0) {
                            $price = $taxHelper->getPrice($this->_product, $this->_product->getPrice(), $includingTax);
                            if ($price > 0) $value = Mage::app()->getStore()->convertPrice($price, true, false);
                            else $value = '';
                        } else {
                            $value = '';
                        }
                    ...
                    case 'special_price':
                        ...
                        if ($this->_product->getData('special_price') > 0 && Mage::app()->getLocale()->isStoreDateInInterval(Mage::app()->getStore(),
                                        $this->_product->getSpecialFromDate(), $this->_product->getSpecialToDate())) {
                            $price = Mage::helper('tax')->getPrice($this->_product,
                                    $this->_product->getData('special_price'), $includingTax);
                            if ($price > 0) $value = Mage::app()->getStore()->convertPrice($price, true, false);
                            else $value = '';
