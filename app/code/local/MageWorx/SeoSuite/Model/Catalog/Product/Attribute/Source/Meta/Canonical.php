            ...
            if ($productId != null) {
                ...
                if ($collection->count()) {
                    foreach ($collection->getItems() as $urlRewrite) {
                        ...
                        if (!isset($this->_options[$urlRewrite->getStoreId() + 2])) {
                            $this->_options[$urlRewrite->getStoreId() + 2] = array('label' => Mage::app()->getStore($urlRewrite->getStoreId())->getName(),
                                'value' => array());
                        }
                        $this->_options[$urlRewrite->getStoreId() + 2]['value'][] = array('value' => $urlRewrite->getIdPath(),
                ...
                if (!$exists) {
                    ...
                    if ($urlRewriteModel->getId()) {
                        $this->_options[$urlRewriteModel->getStoreId() + 2]['value'][] = array('value' => $urlRewriteModel->getIdPath(),
