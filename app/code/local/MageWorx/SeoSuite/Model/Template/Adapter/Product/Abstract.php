        ...
        foreach ($products as $_product) {
            ...
            foreach ($stores as $store) {
                ...
                foreach ($attributes as $attribute) {
                    ...
                    if ($row) {
                        $connection->update($tablePrefix . 'catalog_product_entity_' . $attribute['backend_type'],
                                array('value' => $attributeName),
                                "entity_type_id = $productTypeId AND attribute_id = '$attribute[attribute_id]' AND entity_id = {$product->getId()} AND store_id = {$storeId}");

                        $str = 'UPDATE ' . $tablePrefix . 'catalog_product_entity_' . $attribute['backend_type'] . ' SET ' .
                           'value = ' . $attributeName . ' WHERE ' . "entity_type_id = $productTypeId AND attribute_id = '$attribute[attribute_id]' AND entity_id = {$product->getId()} AND store_id = {$storeId}";

//                        Mage::log($str, null, 'temp.log');
