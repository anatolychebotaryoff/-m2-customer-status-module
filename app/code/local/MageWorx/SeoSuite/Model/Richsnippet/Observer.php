...
class MageWorx_SeoSuite_Model_Richsnippet_Observer
{
    ...
    {
        $helper = Mage::helper('seosuite/richsnippet');
        if (!$helper->isRichsnippetDisabled()) {

            $block = $observer->getBlock();

            if(($helper->isRichsnippetEnabled() && array_key_exists($block->getNameInLayout(), $this->_blockTypes))){
                $injection = true;
            }elseif($helper->isRichSnippetOnlyBreadcrumbsEnabled() && ($block->getNameInLayout() == 'breadcrumbs')){
                $injection = true;
            }
