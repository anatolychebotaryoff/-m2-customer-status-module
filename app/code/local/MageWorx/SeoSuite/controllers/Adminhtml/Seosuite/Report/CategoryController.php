 ...
 */
class MageWorx_Seosuite_Adminhtml_Seosuite_Report_CategoryController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        if (!Mage::helper('seosuite/report')->getCategoryReportStatus()) {
            Mage::getSingleton('adminhtml/session')->addWarning(Mage::helper('seosuite')
            ...
            case 'calculation':
                ...
                if (($current < $total)) {
                    ...
                    if ($current >= $total) {
                        $current        = $total;
                        $result['stop'] = 1;
                        Mage::helper('seosuite/report')->setCategoryReportStatus(1);
    ...
    protected function _prepareCategories()
    {
        ...
        $connection  = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tablePrefix = (string) Mage::getConfig()->getTablePrefix();
        $helper      = Mage::helper('seosuite/report');
    ...
    {
        if (Mage::helper('seosuite/report')->_prepareText($model->getMetaTitle())) {
            return Mage::helper('seosuite/report')->_prepareText($model->getMetaTitle());
        }
    ...
    public function duplicateViewAction()
    {
        if (!Mage::helper('seosuite/report')->getCategoryReportStatus()){
             Mage::getSingleton('adminhtml/session')
                    ->addWarning(Mage::helper('seosuite')->__('You need to generate the report due to recent changes.'));
        }
