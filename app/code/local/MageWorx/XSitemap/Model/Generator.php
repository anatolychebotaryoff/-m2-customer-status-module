...
class MageWorx_XSitemap_Model_Generator
{
    ...
    protected function _init(MageWorx_XSitemap_Model_Sitemap $model, $entityName)
    {
        ...
        $sitemap_disabled_urls = Mage::getStoreConfig('mageworx_seo/google_sitemap/sitemap_disabled_urls');

        if (!empty($sitemap_disabled_urls)) {

          $disabled_urls_tmp = explode("\n", $sitemap_disabled_urls);
          foreach ($disabled_urls_tmp as $key => $val) {
            $val = trim($val);
            if (empty($val)) continue;
            $this->disabledUrls[$key]  = $val;
          }
          //Mage::log($this->disabledUrls, null, "sitemap_xml-urls.log");
    ...
    protected function _generateXmlFromCategories()
    {
        ...
        foreach ($collection as $item) {
            ...
            if (in_array($this->_helper->trailingSlash($url), $this->disabledUrls)) continue;
    ...
    protected function _generateXmlFromProducts()
    {
        ...
        if ($this->_counter < $this->_totalProduct) {
            ...
            foreach ($collection as $item) {
                ...
                if (in_array($url, $this->disabledUrls)) continue;
