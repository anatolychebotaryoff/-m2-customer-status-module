...
class USWF_GiftPurchase_Model_Rule_Condition_Product extends Mage_Rule_Model_Condition_Product_Abstract
{
    /**
    ...
    public function prepareConditionSql()
    {
        ...
        if (($operator == '{}' && $this->getInputType() == 'multiselect') ||
            ($operator == '!{}' && $this->getInputType() == 'multiselect')) {
            $preResult = array();
            foreach ($value as $elem ) {
                $preResult[] = $ruleResource->getOperatorCondition($alias . '.' . $attribute, $operator, $elem);
            }
            $result = ' (' . implode(' AND ', $preResult) . ') ';
        } else {
            $result = $ruleResource->getOperatorCondition($alias . '.' . $attribute, $operator, $value);
        }
