...
if (class_exists('Mage_Sales_Model_Order_Creditmemo_Api')) {
    ...
    class Celigo_Connectorplus_Model_Sales_Order_Creditmemo_Api extends Mage_Sales_Model_Order_Creditmemo_Api {
        ...
        private function createCreditMemo($data = array()) {
            ...
            if ($creditmemo) {
                if (($creditmemo->getGrandTotal() <= 0) && (!$creditmemo->getAllowZeroGrandTotal())) {
                    $this->_fault('data_invalid', self::INCORRECT_DATA_CANNOT_CREATE);
                }
                $comment = '';
        ...
        private function _initCreditmemo($update = false) {
            ...
            try {
                ...
                if (!$order->canCreditmemo()) {
                    ...
                    $this->tempData['message'][] = self::UNABLE_TO_CREATE_CREDITMEMO;
            ...
            if ($creditmemoId) {
                ...
                if (!$order->canCreditmemo()) {
                    $this->tempData['message'][] = self::UNABLE_TO_CREATE_CREDITMEMO;
                    $this->_fault('data_invalid', self::UNABLE_TO_CREATE_CREDITMEMO);
    ...
    class Celigo_Connectorplus_Model_Sales_Order_Creditmemo_Api extends Mage_Sales_Model_Api_Resource {
        const EXPECTED_ARGUMENT_MISSING_MSG = 'Expected argument is_imported is missing';
        const INVALID_ARGUMENT_VALUE_MSG = 'The argument is_imported accepts 0 and 1 only. 0 for not imported and 1 for imported.';
        /**
         * Create credit memo
