...
class Celigo_Connector_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    const XML_PATH_BATCH_ORDER_FLOW_ID = 'connector/othersettings/batchorderflowid';
     ...
     */
    public function addErrorMessageToLog($message = '', $level = '', $storeId = '', $websiteId = '') {
        if ($message != '' && $this->isConnectorLoggingEnabled()) {
            $allowedLevel = $this->getConfigValue(self::XML_PATH_CONNECTOR_LOG_ENABLED, $storeId, $websiteId);
            if ($level == '') {
                $level = $allowedLevel;
            }
