...
class Celigo_Connector_Model_Cron extends Mage_Core_Model_Abstract {
    ...
    const CRON_BATCH_SIZE = 'connector/cronsettings/batchsize';
    ...
    public function pushOrdersToNs($schedule) {
        ...
        try {
            ...
            $this->_helper->addErrorMessageToLog('Batch Process Starting with orders : [' . implode($orderArr, ',') . ']', Zend_Log::INFO);
            ...
            foreach ($orderArr as $orderid) {
                Mage::getModel('connector/connector')->pushOrderToNS($orderid, Celigo_Connector_Helper_Async::TYPE_SYNC, false, true);
            }
            $this->_helper->addErrorMessageToLog('Batch Process Completed.', Zend_Log::INFO);
        }
        catch(Exception $e) {
