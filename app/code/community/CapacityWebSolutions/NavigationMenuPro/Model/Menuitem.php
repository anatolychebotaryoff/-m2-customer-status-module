...
public function getMenuContent($group_id){
	...
	*/
	...
	if(($item->getShowCategoryImage() != 'none') && ($item->getType() == "2")) 
	{
	
		if($item->getShowCategoryImage() == 'thumbnail_image')
		{	
			$cat_id = $item->getCategoryId();
	...
	if(($item->getType() == "3"))
	{
		...
		$html .= '<a rel="'.$this->link_relation.'" class="Level'.$item_menu_level.'" title="'.$this->link_title.'" href="'.$this->url.'"'.$this->item_target.'>';
	...
	*/
	...
	$active = Mage::getModel('cms/block')->setStoreId($current_storeid)->load($item->getStaticblockIdentifier())->getIsActive();
		...
		if($item->getTitleShowHide() == 'show'){
			$html .= '<a rel="'.$this->link_relation.'" class="Level'.$item_menu_level.' '.$this->has_child_element.$child_status.'" title="'.$this->link_title.'" href="'.$this->url.'"'.$this->item_target.'>';
			if(($item->getImageStatus() == '1') && ($item->getImage() != ''))
			{
			$html .= $image_tag;
			}
