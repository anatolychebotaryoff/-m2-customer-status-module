...
class Yotpo_Yotpo_Helper_ApiClient extends Mage_Core_Helper_Abstract
{
	...
	public function prepareProductsData($order) 
	{
                ...
                $products = $order->getAllVisibleItems(); //get all products
		$products_arr = array();

		foreach ($products as $product) {

                    $opt = unserialize( $product->getData('product_options') );

                    $group_name = "";
                    $group_url = "";

                    if (array_key_exists("info_buyRequest", $opt)) {
                        
                        $buyRequest = $opt["info_buyRequest"];
                        if (array_key_exists("grouped-product-name", $buyRequest)) {
                        $group_name = $buyRequest["grouped-product-name"];
                        $group_url = $buyRequest["grouped-product-url"];
                        }
			...
			$product_data['name'] = $group_name ?: $full_product->getName();
			$product_data['url'] = '';
			$product_data['image'] = '';
			try 
			{
				$product_data['url'] = $group_url ?: $full_product->getUrlInStore(array('_store' => $order->getStoreId()));
